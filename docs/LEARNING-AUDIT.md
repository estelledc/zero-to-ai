...
>
> **快照声明（2026-07-10 回写）**：本报告是 33 文件 / 3 条路径时期的历史快照。2.0.0 现状为 45 文件 / 4 条路径（新增 `codex-zero`），F1-F15 待决项均已执行或已决策，逐项处置见 `PROJECT-DEFICIENCIES.md`（附录 C/E/F 为最新状态）。本文仅在对应章节追加「更新」标注，不改写原始结论。
...
> **更新（2026-07-10）**：B.1/B.2 两个顺序问题均已修复——路径现以 preflight → basics → git-basics 开篇（见 PROJECT-DEFICIENCIES 附录 D，LEARNING F2）；Agnes 曾前移至第 6 步，1.1 复审后第三方网关整体移出核心路径，仅保留实验边界页。
...
> **更新（2026-07-10）**：M3 缺口已闭合——`claude-code/first-page.md`（第一 capstone）与 1.1 新增的 `projects/publish-first-site.md`（第二 capstone，发布公开小站）均在路径中，见 PROJECT-DEFICIENCIES 附录 B/D（LEARNING B.3、F1）。
...
> **更新（2026-07-10）**：此问题已修复，两条路径的首页 Card 分别指向 `/paths/` 对应锚点，见 PROJECT-DEFICIENCIES 附录 D（LEARNING F4）。
...
> **更新（2026-07-10）**：前两项阻塞/摩擦缺口已修复——capstone 见 B.3 更新标注；basics 已补 Node.js 具体安装命令、quickstart 已补 checklist 与跨平台等价命令（LEARNING F7/F9）。后两项轻微缺口由 `appendix/troubleshooting.md`（F14）与 verify/glossary 组合覆盖。
...
> **更新（2026-07-10）**：本表为 33 文件历史快照。当前站点 45 文件：claude-code/ 17（新增 first-page、project-guide）、codex/ 8（2.0 新增分区）、methodology/ 9、projects/ 6（新增 publish-first-site）、appendix/ 3（新增 troubleshooting）、根目录 2。六步合规修复统计见 PROJECT-DEFICIENCIES 附录 A。
