...
| 审计文档回写     | 附录 D 清单待执行                      | 仍未执行，已列入 ROADMAP 阶段 1（2.1）文档债消化                          | 附录 D 更新标注                 |
| 人工验证证据     | 1.1 四项 DEFERRED                      | 1.1 与 2.0 共 8 项 DEFERRED，均未闭环，为当前最高优先级缺口               | `RELEASE-CHECKLIST.md`          |
| 指标体系         | 模板与流程已设计                       | 仍无 `metrics/snapshots/` 实际快照，列入 ROADMAP 阶段 0                   | `metrics/README.md`             |
