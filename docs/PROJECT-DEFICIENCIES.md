...
> **更新（2026-07-10）**：附录 D 回写已执行完毕，三份审计报告已带快照声明与逐章节「更新」标注，历史结论未改写。
...
| D-031 | 覆盖范围   | 站点无 Codex / DeepSeek TUI 的任何实质内容，首页 Card 承诺「正在筹备」但无时间线                                                  | `index.mdx` L64-67                                                                                                                                            | P0           | 轻微   | OUT_OF_SCOPE | —                  | ✅ 已修复（Codex 于 2.0 落地；后续工具走 ROADMAP 准入制） | 新                              |
...
| ~~Codex/DeepSeek TUI 无内容~~                        | ~~站点当前聚焦 Claude Code，其他工具为远期规划~~ 已更新：Codex 完整路径于 2.0 落地；其他工具按 `ROADMAP.md` 阶段 2 准入门槛评估，不预先承诺 | D-031        |
...
>
> **更新（2026-07-10）**：上述回写在 2.0.0 发布时仍未执行，三份审计报告继续反映 33 文件 / 3 路径旧快照。此项文档债已列入 `ROADMAP.md` 阶段 1（2.1）的「文档债消化」任务，回写完成后在本清单标记清零。
>
> **清零（2026-07-10）**：本清单全部回写完成——IA/UX/LEARNING 三份报告头部已加快照声明（33 文件 / 3 路径 → 45 文件 / 4 路径），上表所列章节均已追加「更新（2026-07-10）」标注，原始结论保持原样。

## 附录 E：1.1 当前状态（2026-07-10）

本总表记录的是多轮历史审计，不把旧结论删除。1.1 复审新增并实施的高优先级闭环如下：

| 范围             | 旧状态                                             | 1.1 当前状态                                                  | 验证入口                          |
| ---------------- | -------------------------------------------------- | ------------------------------------------------------------- | --------------------------------- |
| 教程协议         | 安装、认证、Hook、Skill、rewind 存在过期或错误指令 | 已按官方一手资料纠正，页面日期与验证矩阵绑定                  | `npm run audit:content-freshness` |
| 进度语义         | 访问即写入完成记录                                 | 访问与完成分离，完成需显式操作，旧 key 保持兼容               | Playwright progress smoke         |
| 端到端测试       | D-029，未实现                                      | 首页、路径、进度、搜索、404、主题、base path、RSS/sitemap     | `npm run test:e2e`                |
| CI 审查产物      | D-032，未实现                                      | PR 上传 `dist` 工件，不建设临时公开预览                       | deploy workflow                   |
| 内容时效性       | D-009/D-010 依赖人工批量维护                       | 具体版本、日期、90 天报告和一手来源矩阵                       | freshness audit                   |
| 依赖风险         | 4 low / 5 moderate                                 | Astro 7 + Starlight 0.41 兼容升级后 0 advisory                | `npm audit`                       |
| Agnes 零成本路线 | 曾作为主路径前置并承诺免费                         | 移出核心路径；旧 URL 仅保留第三方兼容实验、数据风险与恢复边界 | 官方认证页 + provider 当前控制台  |
| D-031 多工具扩展 | 长期项                                             | 保持 1.1 范围外；Codex 仅在 2.0 启动                          | 路线图                            |

## 附录 F：2.0 当前状态与未来方向（2026-07-10）

延续附录 E 的记录方式，只追加当前状态，不改写历史结论。

| 范围             | 1.1 状态                               | 2.0 当前状态                                                                                                        | 验证入口                        |
| ---------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| D-031 多工具扩展 | 范围外长期项，首页 Card 仅「正在筹备」 | Codex 完整零基础路径（8 篇 + `codex-zero`）已落地；后续工具改走准入评估制                                           | `npm run test:codex-docs`       |
| 学习路径         | 3 条                                   | 4 条（新增 `codex-zero`），最终产物、里程碑、完成证据、失败恢复四要素齐备                                           | `learning-paths.ts` + e2e smoke |
| 审计文档回写     | 附录 D 清单待执行                      | ~~仍未执行~~ 已清零（2026-07-10）：三份报告带快照声明与逐章节更新标注                                               | 附录 D 更新标注                 |
| 人工验证证据     | 1.1 四项 DEFERRED                      | 8 项中 2 项线上复查已凭生产站 Playwright 全绿闭环（2026-07-10）；真实账户、Search Console、跨平台共 6 项仍 DEFERRED | `RELEASE-CHECKLIST.md`          |
| 指标体系         | 模板与流程已设计                       | ~~仍无实际快照~~ 首份快照已产出（2026-07-10）；traffic 与 Search Console 数字待所有者补录                           | `metrics/snapshots/`            |

**未来方向**：2.0 之后的阶段规划（证据补齐 → 运营深化 → 第三工具准入 → 平台演进）、新工具准入门槛与全阶段不变量统一记录在 `ROADMAP.md`，本总表不再新增路线图性质的条目。
