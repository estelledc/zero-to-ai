...
- **grep 检查**：用 `grep` 搜关键名字（函数名、变量名、文件名），确认改了的地方和没改的地方都符预期
- **diff 检查**：用 `git diff` 看完整改动，确保没有意外修改
- **实际运行**：如果程序能跑，跑一下看效果；如果不能跑，至少确认语法没问题

完整的验证方法论见[验证方法论](/zero-to-ai/claude-code/verify/)。这里只需要记住原则：**验证不是可有可无的检查项，它是区分"能用"和"用对了"的唯一防线**。
...
**其他排最后**，因为 daily 本来就是垃圾桶。不是贬义 -- 垃圾桶很有用。你不知道放哪的东西就放 daily。但先把有价值的放到它们该去的地方。
...
4. `/sync-all` 刷新：全量重建 HTML 页面，然后在浏览器里 `open index.html` 看成果
...
2. `/daily-learn`：分析今天的对话，补充 daily 中的学习内容
3. `/render daily/<today>.md`：把今天的日报渲染成 HTML（你明天早上用 `open index.html` 就能看到）
...
- [验证方法论](/zero-to-ai/claude-code/verify/) -- 第 5 步"验证"的完整展开。三层检查体系让你不再依赖"感觉应该没问题"。
- [上下文窗口管理](/zero-to-ai/claude-code/context/) -- 理解 Claude Code 的"记忆范围"，学会在对话变慢之前主动干预。
- [学习管理系统](/zero-to-ai/methodology/learning-management/) -- 把日常记录系统化，让每天学到的东西持续复利。
