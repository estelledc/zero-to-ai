...
- 已完成 [10 分钟上手](/claude-code/quickstart/)，能正常启动 `claude`
- 最好已有项目级 [CLAUDE.md](/claude-code/project-guide/)（没有也能跟做，但协作质量会差一截）
- 了解 [Skill 体系](/claude-code/skills/) 的基本概念；文中带 `/` 的自定义 Skill（如 `/commit`、`/wiki`）**不是 Claude Code 内置命令**——没有的话用文中给出的手动替代步骤
...
选好后，若上一任务残留在对话里，先 `/clear`（见[上下文窗口管理](/claude-code/context/)）。
...
- **diff 检查**：用 `git diff --stat` 再 `git diff` 看完整改动，确保没有意外修改
...
2. 跑 `/wiki` lint（若有）：检查知识库健康——有没有写了但没索引的笔记？有没有引了但没消化的来源？没有 wiki 时，手动打开本周笔记扫一遍即可
...
输入 `/wiki` 没反应，以为 Claude Code 坏了。其实那是你（或别人）写的 Skill。解法：先确认 `.claude/skills/<name>/SKILL.md` 是否存在；不存在就用本页的手动替代步骤，需要时再按 [Skill 体系](/claude-code/skills/) 创建。

### 收工不验证就提交

“明天再看”常常变成“再也不看”。解法：至少跑 `git diff --stat` + 一次实际运行（或打开页面），再 commit。细节见[验证方法论](/claude-code/verify/)。
