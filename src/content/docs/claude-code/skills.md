...
在 Claude Code 对话中输入 `/commit`（对应 skill 的 name 字段）。
...
然后创建 `.claude/skills/review.md`，写入以下内容：
...
Claude 会读取你的 Skill 文件并按步骤执行。如果没触发，检查文件路径是否正确——必须在 `.claude/skills/` 目录下。
...
1. 你创建了一个 Skill 叫 `/format`，但输入后 Claude 没反应。你会检查什么？
2. 你想让一个 Skill 在所有项目中都能用，而不是只在当前项目。文件应该放在哪里？
