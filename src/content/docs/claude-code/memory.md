...
Claude Code 内置的记忆能力有两层：你写的 `CLAUDE.md`（项目/用户指令），以及 Claude 自动写入的 auto memory（纠正与偏好）。会话里用 `/memory` 可以查看已加载的 `CLAUDE.md`、开关 auto memory，并打开 auto memory 目录；让 Claude「记住某件事」时，官方默认写入 auto memory，而不是直接改 `CLAUDE.md`。下面 §1-§5 介绍的是在内置能力之上搭建的**自定义记忆索引系统**——它不是 Claude Code 自带的，但能让记忆管理更系统化。如果你刚入门，可以先跳到“动手试一试”用内置功能上手。

### 1. 自建记忆的存储位置

官方 auto memory 存放在 `~/.claude/projects/<项目路径哈希>/memory/` 目录。下面的自定义索引方案可以复用同一目录组织额外条目，也可以放在你自己约定的路径——关键是：三层 `MEMORY-*.md` 索引本身不是官方功能。
