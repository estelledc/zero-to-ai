...
这是零基础路径的第二个实操交付。第一个（[你的第一个 AI 页面](/zero-to-ai/claude-code/first-page/)）让你做出了东西；这一个让你和 AI 的协作质量从"每次重新解释"升级到"打开就能干活"。
...
- 完成了 [你的第一个 AI 页面](/zero-to-ai/claude-code/first-page/)，手里有一个可以工作的项目目录
- 理解 [Skill 系统](/zero-to-ai/claude-code/skills/) 的基本概念（指令文件在 Skill 目录层级中的位置）
- 理解 [记忆系统](/zero-to-ai/claude-code/memory/) 的运作方式（CLAUDE.md 是"每次都读"的记忆，Memory 是"按需检索"的记忆）

如果还没有自己的项目目录，先回到 [first-page](/zero-to-ai/claude-code/first-page/) 创建一个。
...
现在轮到你了。我们为你在 [first-page](/zero-to-ai/claude-code/first-page/) 中创建的项目写一份 CLAUDE.md。

### 1. 打开你的项目

```bash
cd ~/my-first-page    # 替换成你的实际项目目录
```

如果你还记得当时的目录名，直接进去。如果不记得，用 `ls ~` 找一下。
...
保存文件后，关掉当前 Claude Code 会话，重新启动一个新会话：
...
如果 Claude 能准确回答（比如"这是一个个人介绍页面，纯 HTML/CSS"），说明 CLAUDE.md 生效了。如果它含糊其辞，检查文件是否保存在项目根目录、文件名是否大写正确。
...
**从不更新**：写完一次就忘了。CLAUDE.md 应该是活文档——每次 Claude 犯了一个你不想再看到的错，就加一条踩坑记录。
...
1. 你的项目根目录有一个 CLAUDE.md 文件——用 `ls -la` 确认它存在
2. 文件里有一句话项目描述——新 Claude 会话能正确回答"这个项目是做什么的"
3. 常用命令是可以直接复制粘贴执行的——不是"运行构建命令"而是具体的 `open index.html`
4. 踩坑记录来自真实经历——没有编造不存在的问题
5. 至少有一条个人偏好——Claude 不再需要你每次重复的那件事

全部满足？你已经从"能用 AI 做东西"升级到了"能和 AI 高效协作"。接下来学习日常节奏，把这种协作变成每天的工作方式。

## 下一步

- 想深入理解 CLAUDE.md 的设计哲学？→ [CLAUDE.md 编写哲学](/zero-to-ai/methodology/claude-md-philosophy/)，那篇讲"删不掉的就是好规则"测试、100-200 行的 sweet spot、两层架构
- 想把日常使用 Claude Code 变成固定节奏？→ [日常工作节奏](/zero-to-ai/claude-code/daily-rhythm/)，学完这篇你就知道每天开工、收工该做什么
- 想学更复杂的项目指令文件？→ 前面案例中的踩坑表、推送前必查表、Skill 路由表，都在 [配置三层结构](/zero-to-ai/claude-code/config/) 和 [Skill 工程化](/zero-to-ai/methodology/skill-engineering/) 中有更深入的讲解
