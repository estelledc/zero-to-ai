...
这就是上下文窗口在起作用。它不是 bug，是物理限制——就像办公桌不可能无限大。具体有多大？Claude 的上下文窗口大约能容纳 200,000 token——听起来很多，但一个 1000 行的代码文件就占了约 15,000 token，加上对话历史、CLAUDE.md、MCP 工具描述，桌面空间消耗得比你想象的快。
...
从 Claude Code Discord 和 GitHub issues 的统计来看，最常见的初学者困惑不是“怎么装”、“key 在哪填”，而是“为什么 Claude 越聊越笨”。根因几乎都是：一个 session 里做了太多不相关的事，上下文被前面任务的残留信息污染了。
