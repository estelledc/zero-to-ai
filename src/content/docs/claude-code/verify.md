...
- 知道怎么打开终端和输入命令
- 了解 `git` 的基本概念（如果不了解，先看 [Git 10 分钟速成](/appendix/git-basics/)）
...
```powershell
Select-String -Path * -Pattern "processImage" -Recurse -SimpleMatch
```

也可以直接让 Claude 代劳（跨平台最省事）：

```text
请用搜索确认刚才所有新增的函数名、变量名在定义处和调用处拼写一致；把只出现一次的名字列出来。
```

如果 grep 只找到一个结果（只有定义没有调用，或者只有调用没有定义），这就是一个 bug。

**批量检查**：如果改动涉及多个新名词，挨个搜：
...
**如果失败**：名字只出现一次 → 让 Claude 补上缺失的定义或调用，或删掉死代码；拼写不一致 → 统一成正确拼写后再搜一遍确认。
...
**如果 `git diff` 为空**：可能改动已 staged。改用：
...
**如果发现不该改的文件**：先不要 commit。可让 Claude 回退该文件，或用已知安全的 Git 单文件恢复（例如恢复到上次 commit）。恢复前先看 `git diff -- 该文件` 确认你会丢掉什么。
...
Windows 若未安装 `curl`，可用浏览器访问同一 URL，或在 PowerShell 中：

```powershell
Invoke-WebRequest http://localhost:3000/api/your-endpoint
```
...
**如果运行失败**：把完整报错贴给 Claude：

```text
运行失败，完整报错如下：
（粘贴终端或浏览器 Console 的错误）
请定位根因并修复；修完后告诉我如何重新验证。
```

端口被占用时，换端口或结束占用进程后再试；缺依赖时先按项目文档安装，再跑第三层。
...
```text
# 第一层检查
请用搜索确认刚才所有新增的函数名、变量名在定义处和调用处拼写一致。

# 第二层检查
请逐条对比 `git diff` 中的改动和我的原始需求（上面我说过的 prompt），列出来哪些需求你实现了、哪些你可能漏掉了。

# 第三层检查
请列出你可能漏掉的需求或者没有完全按我的意图实现的地方；并给出我可以手动跑的最短验证步骤。
```
...
脚本要在失败时以非零退出码退出，并打印清楚哪一步失败。
```

之后每次 Claude Code 做改动，跑一遍：

```bash
bash verify.sh
```

Windows 上若没有 bash，可让 Claude 同时生成 `verify.ps1`，或在 Git Bash / WSL 中运行 `bash verify.sh`。
...
没有 Git 时仍可做第一层（搜索名字）和第三层（实际运行）。第二层可改为：让 Claude 列出“改了哪些文件、每处为什么改”，你对照原始需求勾选。学 Git 后补上 `git diff` 会更稳，见 [Git 10 分钟速成](/appendix/git-basics/)。
...
```text
请给 src/ 下的入口文件加一行注释，内容是 “// 验证练习 2026-07-10”
如果没有 src/，就改当前目录里最合适的入口文件，并告诉我改了哪个文件。
```
...
```bash
# 第一层：确认注释确实在文件中
grep -rn "验证练习 2026-07-10" .

# 第二层：看 diff 确认只改了预期内容
git diff --stat
git diff

# 第三层：运行程序确认没有破坏任何功能
# 纯 HTML：open / start / xdg-open
# Node.js：npm start 或 npm run dev 或 node your-entry.js
```

勾选：

- □ 第一层：搜索能找到新增标识，且拼写一致
- □ 第二层：`git diff --stat` 的文件列表符合预期，没有陌生文件
- □ 第二层：逐条 diff 能对应到你的原始需求
- □ 第三层：程序能启动（或页面能打开），改动路径被实际点过一次
- □ 你会用“让 Claude 检查 Claude”的三段 prompt 做一次自检
