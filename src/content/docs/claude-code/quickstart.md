...
:::tip[Node.js 不是本页前置]
官方推荐**原生安装**，不再要求先安装 Node.js。只有当你自己的项目需要 npm，或走某些网页发布流程时，才单独安装 Node——详见[通用环境基础设施](/methodology/basics/)的可选小节。
:::
...
**成功标准：** `claude --version` 打印出版本号；`claude doctor` 无致命错误。

原生 Windows 推荐安装 Git for Windows；没有 Git Bash 时 Claude Code 会回退到 PowerShell。WSL 用户应在 WSL 终端内执行 Linux 安装命令，不要在未配置的 CMD 里跑 `install.sh`。
...
API key 会优先于订阅登录。若你本来想用订阅却遇到认证问题，先取消这个环境变量，再用 `/status` 确认当前认证来源：
...
不要把真实 key 提交到 Git，也不要在截图、Issue 或教程示例中公开它。

### 3. 选择工作目录

先用 `pwd`（PowerShell：`Get-Location`）确认你在哪里。新手建议在桌面建一个测试文件夹：
...
**Windows PowerShell（桌面路径因系统语言可能不同）：**
...
**成功标准：** 当前目录出现 `hello.html`，用浏览器打开能看到 Hello World。

- macOS：`open hello.html`
- Windows：`start hello.html`
- Linux：`xdg-open hello.html`
...
- **`claude: command not found`**：安装后重开终端，并运行 `claude doctor`。macOS/Linux 原生安装通常位于 `~/.local/bin/claude`；不要同时保留原生、Homebrew 和旧 npm 全局安装。失败恢复：确认 `~/.local/bin` 在 PATH 中，或按[官方安装页](https://code.claude.com/docs/en/installation)重跑对应平台安装命令。
- **登录后仍提示认证失败**：在 Claude Code 中运行 `/status`。若环境里残留失效的 `ANTHROPIC_API_KEY`，它会优先于订阅登录；按上文取消该变量后重试。
...
- **Claude 第一次写的代码不完美**：这完全正常。告诉它“不对，我要的是 X 而不是 Y”，第二次就会好很多。
- **回复太慢/卡住了**：`Ctrl + C` 中止当前回复，重新发 prompt。
- **旧教程让你先装 Node + npm 全局安装**：那是过时路径。失败恢复：忽略 Node 前置，回到本页第 1 步用原生安装脚本。
...
- [ ] `claude` 能完成浏览器登录，或 `/status` 显示你主动选择的 API 认证
- [ ] 你成功让 Claude 创建了至少一个文件（比如 `hello.html`），并能在浏览器打开
- [ ] 你知道 `/clear` 是干什么用的（切换无关任务时清空上下文）
...
- 或者：用刚学会的 Claude Code 做[你的第一个 AI 页面](/claude-code/first-page/)——从 hello.html 升级成可展示的个人介绍页
- 想先摸清账单再深入？读[成本与计费](/claude-code/cost/)
