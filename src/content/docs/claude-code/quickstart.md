...
:::note[不想先花钱？]
完全没问题。先跟着本页安装 Claude Code，到第 2 步设置 API Key 时跳转 [零成本 vibe coding](/claude-code/agnes-free-vibe-coding/)，用免费模型完成同样的教程。
:::

- 你会打开终端并输入命令（如果不会，先看[通用环境基础设施](/methodology/basics/)）
- 你有 Node.js 和 npm（终端输入 `npm --version`，有版本号输出就行。提示 command not found？macOS 运行 `brew install node`，Windows 运行 `winget install OpenJS.NodeJS.LTS`，装完重开终端）
- 你有一个 Anthropic 账号和 API Key（[console.anthropic.com](https://console.anthropic.com) 获取），或者你打算[用免费模型](/claude-code/agnes-free-vibe-coding/)
...
```powershell
[System.Environment]::SetEnvironmentVariable('ANTHROPIC_API_KEY', '你的API-Key粘贴在这里', 'User')
```

这条命令把 API Key 写入系统环境变量，所有新打开的终端都能看到。

完成后关掉终端重开，验证配置：macOS/Linux 输入 `echo $ANTHROPIC_API_KEY`，Windows 输入 `echo $env:ANTHROPIC_API_KEY`，有输出就表示配置成功了。
...
2. `echo $ANTHROPIC_API_KEY` 有输出（或者你用了 Agnes 免费模型）
