...
项目经理（你对话里的 Claude）理解需求、分解任务、分配给工程师（子 Agent）。工程师各自独立工作 -- 架构师设计 API、前端写 UI、测试写用例 -- 然后项目经理汇总验收。

**技术定义**：子 Agent 是 Claude Code 创建的独立进程，有自己独立的上下文窗口，不共享主对话的历史。每个子 Agent 只拿到你给它的任务描述，看不到你之前的对话 -- 看不到你的 CLAUDE.md、看不到你的记忆系统、看不到前面的讨论。
...
它看不到你之前的对话，看不到你的 CLAUDE.md，看不到你的记忆系统。你给的 prompt 是它拿到的全部信息 -- 遗漏的任何上下文，它都无从得知。
