...
prerequisites:
  ...
  - claude-code/memory
relatedContent:
  - { slug: 'claude-code/memory', label: 'CLAUDE.md 与记忆系统' }
...
**核心原则：让 Claude 知道信息存在哪里，但不占用上下文空间去存储那些用不到的信息。** 这一设计哲学的更多背景详见[设计哲学](/projects/learn-journal/design-philosophy/)。
...
type: feedback
originSessionId: <redacted-session-id>
---
```

`originSessionId` 可以作为本地追溯字段，但真实值不应出现在公开教程、截图或 Issue 中。公开示例应使用明显的占位符：
...
- `feedback_teaching_style.md` 可以在本地记录来源会话，公开时只保留“用户明确提出教学偏好”的脱敏结论
- `feedback_bash_sigpipe.md` 可以记录触发问题的命令模式和日期，不公开可关联到个人会话的标识
...
更重要的是：**git blame 能告诉你每条记忆在什么时候添加、为什么修改**。本地来源字段可以补充追溯；对外发布前必须脱敏。
...
Why：用户在一次教学会话中明确反馈过这个偏好（公开材料不记录内部项目路径和会话标识）
...
3. 本地来源引用（对外发布时移除或替换为 `<redacted-session-id>`）
...
- 继续 [工作流编排思路](/methodology/workflow-design/) —— 理解了记忆系统后，看看怎么把它和其他工具串成完整工作流
- 回顾 [CLAUDE.md 与记忆系统](/claude-code/memory/) —— 重新读一遍，现在你应该能理解 CLAUDE.md 和 Memory 的分工了
- 转到 [Claude Code 上下文窗口管理](/claude-code/context/) —— token 预算怎么分配，记忆占了多少
- 回 [学习方法论](/methodology/)
