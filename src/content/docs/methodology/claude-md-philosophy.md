...
这篇文章不教你 CLAUDE.md 的语法怎么写（那部分参考[配置文档](/zero-to-ai/claude-code/config/)），而是讲一个更根本的问题：**怎么让这本入职手册越写越好，而不是写一次就落灰**。
...
这个模式来自 Boris Cherny（Anthropic 的 Claude Code 产品负责人）。他在一次访谈中说过：他自己项目的 CLAUDE.md 里每一条规则，背后都是一次真实的翻车。Claude 做了不该做的事，他就加上一句话让它以后别再做。
...
来看一个真实的 CLAUDE.md 片段。下面这些规则来自 Jason（零基础编程学习者）用了 8 个月的 intern-journal 项目，每条规则背后都是一次具体的翻车事件。
...
问题不在 Claude，在你。Claude 不会自己学会"以后别再这样做了" -- 它只会读你给的 CLAUDE.md。你不写进去，它永远不知道。
...
Jason 的项目级 CLAUDE.md 当前约 140 行，正好在甜点区内。它覆盖了：
- 项目身份和用户画像（10 行）
- 目录结构与模板约定（30 行）
...
- [CLAUDE.md 配置详解](/zero-to-ai/claude-code/config/) -- 语法、格式、两层架构的配置方法
- [学习方法论总览](/zero-to-ai/methodology/) -- 回到方法论索引
- [记忆系统设计](/zero-to-ai/methodology/memory-system-design/) -- 全局 vs 项目级记忆的深层设计
