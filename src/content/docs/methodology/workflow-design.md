...
AI 编程也是一样。你会攒一堆工具：对话助手负责生成与修改，Skill/模板负责把重复任务标准化，Memory/笔记负责跨会话记忆，git 负责版本控制，pre-commit hook 负责自动检查。**工作流就是把它们按正确的顺序串起来**，让你知道每件事该用哪个工具。

更重要的是，工作流帮你解决一个新问题：当你有五六个 AI 能力可用时，每次打开终端会面临一个“决策瘫痪”局面——从哪开始？该先用哪个？工作流就是把“每次都要想一遍怎么走”变成“看流程图直接走”，决策成本趋近于零。

## 教学模式 vs 实施模式（最核心的一段）

这是 Jason 工作流里最独特的模式——很少有教程单独讲这个环节。它的产生原因很简单：同一个助手，面对“教我这个概念”和“帮我修这个 bug”两个完全不同的任务时，如果用一种风格去应答，用户一定会被逼疯。
...
**AI 编程助手面对的就是这两种场景的矛盾。** 如果没有显式的模式切换，它只会在中间找一个“安全中性”的节奏——教学时讲太快，实施时拖太久。
...
这个模式的项目规则长这样（可写入 `CLAUDE.md` 或 `AGENTS.md`）：

```text
教学节奏：学习类任务默认"小段讲解+即时一问+等回答"，禁一次性丢多题
```

出处是一次真实的用户反馈。Jason 在一次 Hook 功能的学习时对助手说：“用带我学习然后提问的方式，而不是现在这种直接出题目”。这句话变成了全局规则，对所有项目的学习类任务生效。
...
- **教学模式用于实施**：你急着修 bug，助手在那“我们先理解一下这段代码的设计思路，你看这里用了什么设计模式？”——你会想砸电脑。
- **实施模式用于教学**：你想学一个新概念，助手连续写了 200 行代码和 5 段解释一起丢给你——你被淹没了，什么都没学会。

这是一个**几乎所有 AI 编程用户都会踩的坑**。大多数人不自觉地学会了“教的时候自己主动停下来问，干的时候让助手别停”，但从来没把这些规则显式写进项目指令。显式写进去的好处是：换一个会话、换一个项目，助手不需要你重新教它“这次我是学还是做”。

### 实现上不需要复杂的 if-else

不是要你写一个复杂的模式检测规则引擎。只需要两条互斥的规则同时存在于项目指令文件（`CLAUDE.md` / `AGENTS.md`）里，每条带上触发条件。助手读到时会自己判断当前对话属于哪种模式。
...
4. **Memory**：`feedback_bash_sigpipe.md` 记录完整的 Why——pipefail 下 `head` 早退会向上游发 SIGPIPE，导致误报。本地可以保留来源引用，对外教程只保留脱敏后的故障证据。
...
约 15 分钟，把一件你重复做过 3 次以上的事画成迷你工作流：

1. 在纸上（或一个 markdown 文件）写下四格：**触发 → 执行 → 验证 → 记录**
2. 填入具体内容。例如「收工」：触发=准备关电脑；执行=写 daily 三问；验证=文件有今日日期与三条回答；记录=若有新概念再开 `learnings/`
3. 把「教学 vs 实施」两条规则各写一行进 `CLAUDE.md` 或 `AGENTS.md`
4. 下次真做这件事时严格按图走一遍，标出多余或缺失的步骤

**成功标准：** 四格都有可执行内容；项目指令里同时存在教学与实施两条互斥规则。
...
- [记忆系统设计原则](/methodology/memory-system-design/)——工作流里的 Memory 怎么分层
- [CLAUDE.md 编写哲学](/methodology/claude-md-philosophy/)——错误恢复循环的「加规则」方法
- Claude：[Skill 系统](/claude-code/skills/)——怎么写可复用 Skill
- Codex：[AGENTS.md](/codex/agents-md/)——把循环约定写进仓库
- 回 [学习方法论](/methodology/)
