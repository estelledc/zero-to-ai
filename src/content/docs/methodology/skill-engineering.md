...
- 写过至少一个可复用流程（Claude：[Skill](/claude-code/skills/)；或其他工具的固定提示模板）
- 读过[系统提示解剖学](/methodology/prompt-anatomy/)中"分层架构"和"职责隔离"两个模式
- 理解[上下文窗口](/claude-code/context/)的基本限制

如果还没有，先读上面材料。这篇是在"会写流程"基础上的进阶——讨论的不是怎么写好一段提示，而是怎么让它变成可分发的工程资产。
...
**具体例子**：一个「写周报」Skill 不要把「实习周报 / 开源贡献周报 / 团队同步」三种模板全塞进主文件。主文件只写触发条件与路由表；三种模板各放 `references/` 下，按用户说的类型再 Read。
...
**具体例子**：写「产品更新说明」时，契约可以写：意图=让非技术同事 30 秒看懂；签名动作=先一句用户收益再列改动；反模式=堆砌 commit hash、用「赋能/闭环」空话。Agent 按契约自由措辞，而不是填空模板。
...
约 20 分钟，把你现有的一个「长提示 / 单文件 Skill」改成最小工程化形态：

1. 新建目录：`SKILL.md`（或 `README.md` 入口）+ `references/`（先放 1–2 个场景文件即可）
2. 在入口写**路由表**：用户意图 → 该读哪个 reference（不要让 Agent 猜）
3. 给入口加一节「不做什么」+ 一个硬 checkpoint（例如「生成最终稿前必须逐项确认标题与受众」）
4. 列 3 条该领域的 Anti-AI-Slop（默认平庸行为 + 例外条件）
5. （可选）加 `manifest.json` 的 name / version 字段，方便以后分发

**成功标准：** 入口文件短于原来的一半；至少有一个 reference 被路由表显式指向；checkpoint 与反默认列表都写在文件里而不是只在你脑子里。

## Checkpoint

- □ 我能说明 L1/L2/L3（描述 / 正文 / references）的加载时机
- □ 我理解「主题即契约」：意图 + 签名 + 反模式，而不是死板模板
- □ 我知道重要决策要逐项 checkpoint，不能打包成一个 yes/no
- □ 我完成了上面的「动手试一试」，至少拆出过一个 reference
- □ 我能判断何时主 Agent 自查、何时需要独立视角（SubAgent / 第二轮审查）

## 下一步

- [Skill 体系](/claude-code/skills/) — Claude 侧从零写 Skill
- [系统提示解剖学](/methodology/prompt-anatomy/) — 配套的 5 个设计模式
- [学习日志 Skill Pack](/methodology/skill-pack/) — 可对照的工程化产物（含 Codex 适配器）
- [子 Agent 并行](/claude-code/subagents/) — 自检分层中的 SubAgent 细节
