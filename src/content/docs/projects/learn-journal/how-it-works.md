...
这张表是知识捕获的路由——mentor 反馈、code review 等外部反馈走单独的反馈管理流程（`feedback/inbound/` → `feedback/action/`），不在这张表里。

这是简化版——完整的工件选择逻辑见[学习管理系统](/methodology/learning-management/)，其中还包括 feedback 类工件的路由。
